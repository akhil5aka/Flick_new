...
export const writeSuccessMessage = async (
  ...
  const filename =
    moment().format("YYMMDDHHmmssSSSS") +
    "_response" +
    /*cleanupFilename(invoiceNumber) +*/ ".json";

  writeFileSync(
    path.join(process.env.OUT_FOLDER ?? "OUT", filename),
    JSON.stringify(data, null, 4).replace(/\n/g, "\r\n"),
    "utf-8"
  );
};

export const writeValidationErrorMessage = (
  invoiceNumber: string,
  errorMessage: string
) => {
